...
import { taskInput,tasksObj,plusIcon,ENTER_KEY_CODE,tasks,STYLES } from "./variables.js";
...
tasks.addEventListener('click', function crossRowRemove(event) {
    ...
    const taskEl = tasksObj.find(
        (item) => item.crossRowId === target.id
    );

    if (!isCrossRow || (event.target.id !== taskEl.crossRowId) ) {
        return;
    }
